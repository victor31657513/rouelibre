...
validate_canonical_preview_path() {
  local canonical_path="$1"
  if [[ -z "$canonical_path" ]]; then
    fail "Canonical PREVIEW_PATH must not be empty."
  fi
  if contains_control_tab_newline_or_space "$canonical_path"; then
    fail "Canonical PREVIEW_PATH must not contain spaces, control characters, tabs, or newlines."
  fi
  case "$canonical_path" in
    /var/www/*) ;;
    *) fail "Canonical PREVIEW_PATH must be located below /var/www/." ;;
  esac
  if [[ "$canonical_path" == "/var/www" ]]; then
    fail "Canonical PREVIEW_PATH must not be /var/www."
  fi
}

if [[ "${1:-}" == "--canonical-path" ]]; then
  if [[ $# -ne 2 ]]; then
    fail "Usage: validate-preview-deployment.sh --canonical-path PATH"
  fi
  validate_canonical_preview_path "$2"
  exit 0
fi
