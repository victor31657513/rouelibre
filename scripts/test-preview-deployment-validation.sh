...
"$validator" --canonical-path "/var/www/roue-libre-preview"

set +e
"$validator" --canonical-path "/" >/tmp/preview-validation.out 2>/tmp/preview-validation.err
canonical_status=$?
set -e
if [[ $canonical_status -eq 0 ]]; then
  echo "Expected canonical path outside /var/www/ to fail" >&2
  exit 1
fi
