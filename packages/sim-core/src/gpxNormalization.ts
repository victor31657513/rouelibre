...
export interface GpxHorizontalPositionNormalizationReport {
  readonly sourcePointCount: number;
  readonly retainedPointCount: number;
  readonly removedPointCount: number;
  readonly removedExactDuplicatePointCount: number;
  readonly removedDifferentAltitudePointCount: number;
  readonly removedSourcePointIndices: readonly number[];
  readonly removedExactDuplicateSourcePointIndices: readonly number[];
  readonly removedDifferentAltitudeSourcePointIndices: readonly number[];
}

export interface GpxHorizontalPositionNormalizationResult {
  readonly track: ParsedGpxTrack;
  readonly report: GpxHorizontalPositionNormalizationReport;
}
...
/** Conserve la première occurrence de chaque position horizontale consécutive exacte. */
export function removeConsecutiveSameHorizontalGpxPoints(
  track: ParsedGpxTrack,
): GpxHorizontalPositionNormalizationResult {
  if (typeof track !== "object" || track === null) throw new TypeError("track must be an object");
  if (!Array.isArray(track.points)) throw new TypeError("track.points must be an array");
  if (track.points.length < 2) throw new RangeError("track.points must contain at least two points");
  for (let index = 0; index < track.points.length; index += 1) assertPoint(track.points[index], index);
  ...
  const retainedPoints: Readonly<RawGpxTrackPoint>[] = [copyPoint(first)];
  const removedSourcePointIndices: number[] = [];
  const removedExactDuplicateSourcePointIndices: number[] = [];
  const removedDifferentAltitudeSourcePointIndices: number[] = [];
  let previousRetained = first;
  ...
  for (let index = 1; index < track.points.length; index += 1) {
    ...
    if (previousRetained.latitudeDegrees === current.latitudeDegrees
      && previousRetained.longitudeDegrees === current.longitudeDegrees) {
      removedSourcePointIndices.push(index);
      if (previousRetained.altitudeMeters === current.altitudeMeters) {
        removedExactDuplicateSourcePointIndices.push(index);
      } else {
        removedDifferentAltitudeSourcePointIndices.push(index);
      }
    } else {
      retainedPoints.push(copyPoint(current));
      previousRetained = current;
  ...
  if (retainedPoints.length < 2) {
    throw new RangeError("horizontal position normalization must retain at least two points");
  }
  const frozenRemovedIndices = Object.freeze(removedSourcePointIndices);
  const frozenExactIndices = Object.freeze(removedExactDuplicateSourcePointIndices);
  const frozenDifferentAltitudeIndices = Object.freeze(removedDifferentAltitudeSourcePointIndices);
  const normalizedTrack = Object.freeze({ points: Object.freeze(retainedPoints) });
  const report = Object.freeze({
    sourcePointCount: track.points.length,
    retainedPointCount: retainedPoints.length,
    removedPointCount: removedSourcePointIndices.length,
    removedExactDuplicatePointCount: removedExactDuplicateSourcePointIndices.length,
    removedDifferentAltitudePointCount: removedDifferentAltitudeSourcePointIndices.length,
    removedSourcePointIndices: frozenRemovedIndices,
    removedExactDuplicateSourcePointIndices: frozenExactIndices,
    removedDifferentAltitudeSourcePointIndices: frozenDifferentAltitudeIndices,
  });
  return Object.freeze({ track: normalizedTrack, report });
}
