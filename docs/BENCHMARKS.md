...
Le banc fonctionnel exécute la référence neutre `identity` sur 14 profils synthétiques, 21 traces canoniques complètes et 43 cas du manifeste. Les 20 invariants éliminatoires réussissent et les 34 métriques ordonnées confirment des corrections exactement nulles ainsi que la conservation exacte des distances, altitudes, pentes, dénivelés positifs et dénivelés négatifs. Le rapport complet reste en mémoire et le dépôt conserve uniquement son résumé compact avec empreinte SHA-256. Cette exécution ne mesure aucune performance et ne constitue ni une stratégie candidate ni une comparaison de méthodes.
