...
Le [corpus des extrêmes altimétriques](ALTIMETRIC_EXTREMES_CORPUS.md) matérialise deux critères exhaustifs : variation absolue strictement supérieure à `20 m` (`29` segments) et pente brute absolue strictement supérieure à `1` (`12` segments). Leur intersection contient `2` segments et leur union `39`. Les observations source aux percentiles `99`, `99,9` et `100` des deux distributions forment six motifs avant déduplication ; elles ajoutent `4` cas absents de l’union des seuils, pour un total de `43` cas uniques.

## Référence altimétrique identité

Le banc fonctionnel exécute la référence neutre `identity` sur 14 profils synthétiques, 21 traces canoniques complètes et 43 cas du manifeste. Chacun des 20 invariants éliminatoires est évalué et fournit une preuve déterministe. Les 34 métriques ordonnées restent le contrat de mesure du protocole. Une section distincte du rapport compare exactement, profil par profil, la cardinalité, chaque distance, chaque altitude, chaque pente d'intervalle et les deux dénivelés. Tous les indicateurs de conservation du résumé sont dérivés de ces comparaisons de séquences complètes, sans inférence à partir d'extrema, de percentiles, de cas isolés ou du seul nombre de points. L'invariant protégeant les coordonnées source compare leurs instantanés avant et après l'exécution sans aligner la source sur la sortie préparée ; la traçabilité de chaque point préparé reste validée séparément. Le rapport complet, déterministe et profondément immuable, reste en mémoire et le dépôt conserve uniquement son résumé compact avec empreinte SHA-256. Cette exécution ne mesure aucune performance et ne constitue ni une stratégie candidate ni une comparaison de méthodes.

Le corpus compte `20` segments montants et `23` descendants ; `2` cas ont aussi un espacement horizontal strictement supérieur à `250 m`. La répartition par étape est : étape 2 (`1`), étape 3 (`12`), étape 6 (`5`), étape 15 (`5`), étape 17 (`4`), étape 18 (`1`), étape 19 (`2`) et étape 20 (`13`). Chaque cas conserve tous ses motifs ordonnés, ses indices bruts et normalisés, ses métriques non arrondies et jusqu’à deux points de contexte de chaque côté. Le test dédié reconstruit exactement le JSON depuis les 21 GPX bruts sans les modifier. Ces seuils restent diagnostiques et ne constituent aucun paramètre de préparation.
