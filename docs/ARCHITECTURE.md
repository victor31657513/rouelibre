...
- `src/precompiledCourse.ts` contient une configuration de parcours précompilée indépendante : échantillons ordonnés de distance et d’altitude en mètres, validation, copie défensive, immutabilité et interpolation linéaire d’altitude. Ce module ne connaît ni GPX ni coordonnées GPS et dépend uniquement du langage JavaScript. Il ne dépend ni de la physique, de l’énergie ou du rendu, ni de React, Three.js, du DOM ou du navigateur.
