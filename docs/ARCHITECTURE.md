...
- `src/gpxNormalization.ts` copie une `ParsedGpxTrack` et supprime uniquement ses doublons consécutifs exacts avant le calcul des distances. Cette passe ne dépend ni de la géodésie, ni de la physique, ni du laboratoire et produit une trace ainsi qu’un rapport profondément immuables.
...
- `removeConsecutiveExactGpxDuplicates` transforme une `ParsedGpxTrack` en nouvelle trace normalisée avant `computeGpxCumulativeDistances` et rapporte les effectifs ainsi que les indices source supprimés.
...
GPX XML brut → ParsedGpxTrack → DistanceAnnotatedGpxTrack → GpxGeometryQualityReport → décision future de normalisation → PrecompiledCourse futur → LongitudinalCourse → simulation
```
