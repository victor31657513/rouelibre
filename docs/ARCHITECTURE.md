...
- `src/gpx.ts` définit la frontière des données source GPX brutes et analyse un sous-ensemble GPX 1.1 déterministe (une trace, un segment, au moins deux points). Il dépend uniquement d’ES2022, sans DOM, `DOMParser`, navigateur, système de fichiers, réseau, physique ou énergie. Il conserve latitude et longitude en degrés et altitude en mètres, sans conversion vers un parcours précompilé ou consommable.
...
GPX source → (conversion future absente) → PrecompiledCourse → LongitudinalCourse → simulation
```
