...
Le workflow se déclenche manuellement ou lors d’un push sur `main`. Une fusion automatique produite avec le `GITHUB_TOKEN` ne déclenche pas le workflow `push` ; l'exécution de `.github/workflows/enable-auto-merge.yml` qui demande l'auto-merge attend donc l'état `MERGED` pendant au plus 120 secondes. Après confirmation et en l'absence d'une exécution pour le commit de fusion, elle déclenche explicitement `deploy-pages.yml` sur `main` par `workflow_dispatch`, puis vérifie qu'une telle exécution couvrant ce commit existe. L'événement `closed` reste un secours indépendant du chemin principal. Une Pull Request non fusionnée ne publie aucun déploiement.
