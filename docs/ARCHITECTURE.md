...
- Le [protocole JSON de comparaison altimétrique](../data/courses/tour-de-france/2026/altimetry-comparison-protocol.json) et son test spécifique aux données restent hors de la boucle de simulation. Ils ne sont pas consommés par `apps/lab`, ne constituent ni une API ni une configuration de production et ne fournissent aucune stratégie. Le futur banc d'essai restera séparé de la préparation de production et de sa conversion vers `PrecompiledCourse`, conformément à la [décision 0008](decisions/0008-protocole-comparaison-preparation-altimetrique.md).
