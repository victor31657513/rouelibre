...
- `src/course.ts` contient le domaine de parcours longitudinal : validation, copie défensive, fabrique de parcours fini plat depuis l’origine 0 m, segments ordonnés immuables, longueur totale optionnelle, bornage à l’arrivée, progression pure et résolution binaire de pente depuis une distance. Il ne dépend ni de la physique, ni de l’énergie, ni de React, du DOM ou du navigateur.
