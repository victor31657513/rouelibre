...
- Banc d'essai altimétrique neutre, déterministe et limité aux tests : la référence non candidate `identity` applique les 34 métriques aux 14 profils synthétiques, aux 21 traces complètes et aux 43 cas. Les 20 invariants sont calculés individuellement dans l'ordre du protocole, accompagnés d'une preuve déterministe et résumés depuis le rapport réel ; l'invariant des coordonnées compare des instantanés de la source indépendamment de la cardinalité préparée. Une section dédiée du rapport compare exactement, pour chaque profil, les séquences complètes de distances, d'altitudes et de pentes ainsi que les dénivelés positif et négatif ; les indicateurs de conservation du résumé reposent exclusivement sur ces preuves et non sur les 34 métriques agrégées. La mesure aux distances source accepte un profil préparé comportant des abscisses différentes grâce à une interpolation linéaire sans extrapolation. Le rapport exhaustif est comparé uniquement en mémoire ; seul son résumé compact dérivé et son empreinte SHA-256 sont versionnés.
...
- Workflow GitHub Actions unique pouvant publier le build Vite du laboratoire sur GitHub Pages depuis `main`, après installation, typecheck, tests et build de production ; dans la même exécution que la demande d'auto-merge, le workflow d'orchestration attend `MERGED` pendant au plus 120 secondes, déclenche explicitement le déploiement par `workflow_dispatch` sauf si une exécution couvre déjà le commit de fusion, puis vérifie la création effective de cette exécution.
