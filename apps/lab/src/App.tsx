...
export function App({ simulation: providedSimulation }: AppProps): React.JSX.Element {
  ...
  const runner = useMemo(() => new FixedStepRunner({ simulation, onAfterStep: () => { const nextSnapshot = simulation.getSnapshot(); setSnapshot(nextSnapshot); if (nextSnapshot.courseProgress.isFinished === true) setRunning(false); }, onError: (err) => { setRunning(false); setError(err instanceof Error ? err.message : String(err)); } }), [simulation]);
  ...
  const start = (): void => { if (snapshot.courseProgress.isFinished === true) return; setError(undefined); runner.start(); setRunning(true); };
  ...
  return <main className="app-shell">
    ...
    <section className="panel controls" aria-labelledby="controls-title"><h2 id="controls-title">Commandes</h2>
      <label>Puissance demandée <input aria-label="Puissance demandée" type="range" min="0" max="1200" step="5" value={physical.requestedPowerWatts} disabled={isFinished} onInput={(event) => setPower(event.currentTarget.valueAsNumber)} onChange={(event) => setPower(event.currentTarget.valueAsNumber)} /> <output>{fmt(physical.requestedPowerWatts, 0)} W</output></label>
      <label>Vent longitudinal <input aria-label="Vent longitudinal" type="range" min="-10" max="10" step="0.5" value={snapshot.environment.windSpeedMetersPerSecond} disabled={isFinished} onInput={(event) => setWind(event.currentTarget.valueAsNumber)} onChange={(event) => setWind(event.currentTarget.valueAsNumber)} /> <output>{fmt(snapshot.environment.windSpeedMetersPerSecond, 1)} m/s</output></label>
    ...
    <section className="grid" aria-label="Observables">
      ...
      <article className="panel"><h2>Parcours</h2><dl><dt>Mode</dt><dd>{isConstantCourse ? "Pente constante" : "Parcours segmenté de démonstration"}</dd><dt>Longueur totale</dt><dd>{courseProgress.totalLengthMeters === undefined ? "Aucune arrivée" : `${fmt(courseProgress.totalLengthMeters)} m`}</dd><dt>Ligne d’arrivée</dt><dd>{courseProgress.totalLengthMeters === undefined ? "Aucune" : `${fmt(courseProgress.totalLengthMeters)} m`}</dd><dt>Distance restante</dt><dd>{courseProgress.remainingDistanceMeters === undefined ? "Non définie" : `${fmt(courseProgress.remainingDistanceMeters)} m`}</dd><dt>Progression</dt><dd>{courseProgress.progress === undefined ? "Non définie" : `${fmt(courseProgress.progress * 100, 1)} %`}</dd><dt>État</dt><dd>{isFinished ? "Arrivé" : (courseProgress.isFinished === false ? "En course" : "Sans arrivée")}</dd><dt>Segment actif</dt><dd>Segment {coursePosition.segmentIndex + 1} / {coursePosition.segmentCount} — {formattedRoadGradePercent} %</dd><dt>Début du segment</dt><dd>{fmt(coursePosition.segmentStartDistanceMeters)} m</dd><dt>Distance dans le segment</dt><dd>{fmt(coursePosition.distanceIntoSegmentMeters)} m</dd><dt>Prochaine frontière</dt><dd>{coursePosition.nextSegmentStartDistanceMeters === undefined ? "dernier segment" : `${fmt(coursePosition.nextSegmentStartDistanceMeters)} m`}</dd></dl></article>
    </section>
  </main>;
}
