...
export function App({ simulation: providedSimulation }: AppProps): React.JSX.Element {
  ...
  return <main className="app-shell">
    ...
    <section className="panel controls" aria-labelledby="controls-title"><h2 id="controls-title">Commandes</h2>
      ...
      <label>Pente longitudinale <input aria-label="Pente longitudinale" type="range" min="-20" max="20" step="0.5" value={roadGradePercent} onInput={(event) => setRoadGradePercent(event.currentTarget.valueAsNumber)} onChange={(event) => setRoadGradePercent(event.currentTarget.valueAsNumber)} /> <output>{formattedRoadGradePercent} %</output></label>
    ...
    <section className="panel visual" aria-labelledby="visual-title"><h2 id="visual-title">Route à pente constante</h2><div className="road"><div className="rider" style={{ left: `${riderPercent}%` }} aria-label="Position visuelle du coureur" /></div><div className="gauge" aria-label="Jauge W prime"><span style={{ width: `${reservePercent}%` }} /></div><p>Réserve W′ : {fmt(energy.anaerobicReserveJoules, 0)} J ({fmt(reservePercent, 1)} %)</p></section>
