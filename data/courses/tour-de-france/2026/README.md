...
Le fichier `altimetry-comparison-protocol.json` définit les entrées, invariants, conventions de mesure, métriques et rapports d'une future comparaison reproductible. Il impose de préparer les 21 traces complètes puis d'extraire les résultats des 43 cas dans l'ordre du manifeste : aucun cas n'est traité comme un fragment isolé. Ce protocole hors production ne contient ni stratégie candidate ni paramètre de préparation.

Le fichier `altimetry-identity-summary.json` contient uniquement la synthèse compacte et l'empreinte SHA-256 de l'exécution de la référence neutre `identity`. Le banc d'essai reconstruit, valide deux fois et compare le rapport fonctionnel complet exclusivement en mémoire ; aucune métrique détaillée par trace ni donnée point par point n'est versionnée.
